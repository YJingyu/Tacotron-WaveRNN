...
template <class T> inline T sigmoid_d(const T x) {
    if (-2.5 > x) return 0;
    else if (2.5 < x) return 1;
    else return 0.2 * x + 0.5;
}

template <class T> inline T tanh_d(const T x) {
    if (-1 > x) return -1;
...
template <class T1, class T2, class T3, int r, int ci, int ch> void gru(const T1 &x, T2 &h, const T3 (&wi)[r][ci], const T3 (&wh)[r][ch], const T3 (&bi)[r], const T3 (&bh)[r]) {
    ...
    for (int i = 0; i < HIDDEN_SIZE; i++) {
        reset_gate[i] = sigmoid_d(igates[i] + hgates[i]);
        input_gate[i] = sigmoid_d(igates[HIDDEN_SIZE + i] + hgates[HIDDEN_SIZE + i]);
        new_gate[i]   = tanh_d(igates[HIDDEN_SIZE * 2 + i] + (reset_gate[i] * hgates[HIDDEN_SIZE * 2 + i]));
